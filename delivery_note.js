	...
	refresh: function(doc, dt, dn) {
		...
		if ((!doc.is_return) && (doc.status!="Closed" || this.frm.is_new())) {
			if (this.frm.doc.docstatus===0) {
				...
				// 		})
				// 	}, __("Get Items From"));
               this.frm.add_custom_button(__('Loading Advice'),
					function() {
						if (!me.frm.doc.customer) {
							frappe.throw({
								title: __("Mandatory"),
						...
						erpnext.utils.map_current_doc({
							method: "erpnext.selling.doctype.sales_order.sales_order.make_delivery_note_of_loading",
							source_doctype: "Loading Advice",
							target: me.frm,
							setters: {
								customer: me.frm.doc.customer,
							},
