...
@frappe.whitelist()
def make_delivery_note_of_loading(source_name, target_doc=None, skip_item_mapping=False):
	def set_missing_values(source, target):
		target.ignore_pricing_rule = 1
		target.run_method("set_missing_values")
		target.run_method("set_po_nos")
	...
	if not skip_item_mapping:
		mapper["Loading Advice Detail"] = {
			"doctype": "Delivery Note Item",
			"field_map": {
				"rate": "rate",
				"item_code": "so_detail",
				"parent1": "against_sales_order",
			},
			"postprocess": update_item,
			"condition": lambda doc:doc.delivered_by_supplier!=1
		}

	target_doc = get_mapped_doc("Loading Advice", source_name, mapper, target_doc, set_missing_values)
